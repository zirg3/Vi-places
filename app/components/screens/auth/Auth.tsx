        ...
        <Layout>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className={styles.wrapper}>
                    {isReg && (
                        <>
                            <input className={styles.input} {...register('login', {required: true, minLength: 3})} type="text" placeholder='Логин'/>
                            {errors.login?.type === 'required' && <span className={styles.error}>Заполните поле</span>}
                            {errors.login?.type === 'minLength' && <span className={styles.error}>Логин не менее 3 символов</span>}
                        </>
                    )}
